...
export default function Funcionarios() {
    ...
    async function getList() {
        await ServiceFuncionarios.list(value).then(res => {
    ...
    async function handleSubmit() {
        await ServiceFuncionarios.save(entity).then(res => {
            getList();
            setResponse(res);
            setLoading(true);
        });
    ...
    async function handleEdit() {
        await ServiceFuncionarios.update({id: entityEdit.id, ...entity}).then(res => {
            getList();
            setResponse(res);
            setLoading(true);
        });
    ...
    async function handleDelete(entity) {
        await ServiceFuncionarios.delete(entity).then(res => {
            getList();
            setResponse(res);
            setLoading(true);
        });
    ...
    return (
        <Spin spinning={loading}>
            <h2 className={"title-screen"}>Funcionários</h2>
