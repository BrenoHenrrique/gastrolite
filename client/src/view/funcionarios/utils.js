...
export default function ModalBody({entityEdit = null, entity = null}) {
    ...
    useEffect(() => {
        if (entityEdit) {
            const {celular, nome, endereco, funcao} = entityEdit;
            setCelular(celular);
            setNome(nome);
    ...
    useEffect(() => {
        if (entityEdit === null) {
            if ((celular && celular !== "") && (nome && nome !== "") && (endereco && endereco !== "") && (funcao && funcao !== "")) {
                entity({
                    celular: celular,
                    nome: nome,
                    endereco: endereco,
    ...
    return (
        <Row gutter={[24, 8]}>
            ...
            <Col span={24}>
                <label style={{paddingLeft: "3px"}}>FUNÇÃO</label>
            </Col>
            <Col span={24}>
                <Input
                    name={"funcao"}
